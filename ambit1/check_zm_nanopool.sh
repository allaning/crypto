...
# */5 * * * * /home/allan/git/allaning/crypto/ambit1/check_zm_nanopool.sh >/dev/null 2>&1
...
LOG=/home/allan/`date +%Y%m%d_%H%M%S`_check_zm_nanopool.log
...
# The following must be updated if not using zm

if (($RESTART_MINER == 1)); then
  echo Restarting miner! Log: $LOG
  # Kill the processes
  pkill -f "zm_0.5.8/miner"
  # Pause
  sleep 10  # seconds
  # Restart miner
  cd /home/allan/
  screen -dmS zecm ./start_zm_nanopool.bash
fi
